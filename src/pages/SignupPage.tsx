...
import { User, Mail, Lock, UserPlus, AtSign, CheckCircle2, ArrowRight } from 'lucide-react';
...
export const SignupPage: React.FC<SignupPageProps> = ({ onSignupSuccess, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success && res.user) {
      ...
      setError(
        res.error || 'Failed to create account. Please check your details.'
      );
    }
  };
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white/95 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-2xl backdrop-blur-xl">
        {createdUser ? (
          ...
          <div className="text-center py-4 space-y-6 animate-fade-in">
            <div className="w-16 h-16 rounded-3xl bg-emerald-500/10 text-emerald-500 mx-auto flex items-center justify-center border border-emerald-500/20 shadow-lg shadow-emerald-500/10">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-emerald-600 dark:text-emerald-400 mt-1">
                Welcome to TypeFast, {createdUser.name || createdUser.username}.
              </p>
            </div>

            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              Your profile and typing analytics dashboard are ready. Start tracking your speed, climbing leaderboards, and earning certificates.
            </p>

            <button
              onClick={handleContinueToDashboard}
              className="btn-interactive w-full py-3.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-bold text-sm shadow-md shadow-brand-500/25 flex items-center justify-center gap-2 cursor-pointer"
            >
